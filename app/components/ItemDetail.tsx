'use client';

import Link from 'next/link';
import { useLanguage } from '@/app/providers/LanguageProvider';
import { config } from '@/app/config/store';
import Navbar from '@/app/components/Navbar';

interface ItemDetailProps {
    item: {
        id: number;
        name: string;
        description: string;
        fullDescription: string;
        image: string;
        price: number;
        category: string;
        stock: number;
    };
}

export default function ItemDetail({ item }: ItemDetailProps) {
    const { t } = useLanguage();

    const whatsappMessage = `Hi! I'm interested in this product:%0A%0A*${item.name}*%0APrice: $${item.price}%0ACategory: ${item.category}%0A%0APlease provide more information.`;
    const whatsappLink = `https://wa.me/${config.whatsappNumber}?text=${whatsappMessage}`;

    return (
        <div className="min-h-screen bg-gray-50">
            <Navbar />

            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <Link
                    href="/"
                    className="inline-flex items-center text-orange-500 hover:text-orange-600 mb-8 font-medium"
                >
                    <svg className="w-5 h-5 mr-2 rtl:ml-2 rtl:mr-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                    </svg>
                    {t('backToStore')}
                </Link>

                <div className="bg-white rounded-lg shadow-lg overflow-hidden">
                    <div className="md:flex">
                        <div className="md:w-1/2 h-96 md:h-auto bg-gray-200">
                            <img
                                src={item.image}
                                alt={item.name}
                                className="w-full h-full object-cover"
                            />
                        </div>

                        <div className="md:w-1/2 p-8">
                            <div className="mb-4">
                                <span className="inline-block bg-orange-100 text-orange-600 px-3 py-1 rounded-full text-sm font-medium">
                                    {item.category}
                                </span>
                            </div>

                            <h1 className="text-4xl font-bold text-gray-900 mb-4">{item.name}</h1>

                            <div className="flex items-center mb-6">
                                <span className="text-4xl font-bold text-orange-500">${item.price}</span>
                                <span className="ltr:ml-4 rtl:mr-4 text-gray-600">
                                    {item.stock > 0 ? (
                                        <span className="text-green-600">✓ {t('inStock')} ({item.stock} {t('available')})</span>
                                    ) : (
                                        <span className="text-red-600">{t('outOfStock')}</span>
                                    )}
                                </span>
                            </div>

                            <div className="mb-8">
                                <h2 className="text-2xl font-bold text-gray-800 mb-3">{t('productDescription')}</h2>
                                <p className="text-gray-700 leading-relaxed">{item.fullDescription}</p>
                            </div>

                            <div className="space-y-4">
                                <a
                                    href={whatsappLink}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="w-full bg-green-500 text-white px-8 py-4 rounded-lg hover:bg-green-600 transition-colors font-bold text-lg flex items-center justify-center gap-3"
                                >
                                    <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                                        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                                    </svg>
                                    {t('orderViaWhatsApp')}
                                </a>
                            </div>

                            <div className="mt-8 border-t pt-6">
                                <h3 className="font-bold text-gray-800 mb-3">{t('productDetails')}</h3>
                                <ul className="space-y-2 text-gray-700">
                                    <li className="flex justify-between">
                                        <span>{t('category')}:</span>
                                        <span className="font-medium">{item.category}</span>
                                    </li>
                                    <li className="flex justify-between">
                                        <span>{t('itemId')}:</span>
                                        <span className="font-medium">#{item.id}</span>
                                    </li>
                                    <li className="flex justify-between">
                                        <span>{t('availability')}:</span>
                                        <span className="font-medium">{item.stock} {t('units')}</span>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
            </main>
        </div>
    );
}
